...
class GeneralHelper
{
     ...
     * determine penalty
     */
    public static function determine_penalty_rate($id)
    {
        $loan = Loan::find($id);
        $charge_id = LoanProductCharge::where('loan_product_id', $loan->loan_product_id)->first()->charge_id;
        $charge = Charge::find($charge_id);
        $penalty = 0;
        if ($charge->product == 'loan' && $charge->penalty == 1) {
            if ($loan->repayment_cycle == 'annually') {
                //return the penalty per year
                if ($charge->penalty_period == 'year') {
                    $penalty = $charge->amount;
                }
                if ($charge->penalty_period == 'month') {
                    $penalty = $charge->amount * 12;
                }
                if ($charge->penalty_period == 'week') {
                    $penalty = $charge->amount * 52;
                }
                if ($charge->penalty_period == 'day') {
                    $penalty = $charge->amount * 365;
                }
            }
            if ($loan->repayment_cycle == 'semi_annually') {
                //return the penalty per semi annually
                if ($charge->penalty_period == 'year') {
                    $penalty = $charge->amount / 2;
                }
                if ($charge->penalty_period == 'month') {
                    $penalty = $charge->amount * 6;
                }
                if ($charge->penalty_period == 'week') {
                    $penalty = $charge->amount * 26;
                }
                if ($charge->penalty_period == 'day') {
                    $penalty = $charge->amount * 182.5;
                }
            }
            if ($loan->repayment_cycle == 'quarterly') {
                //return the penalty per quaterly

                if ($charge->penalty_period == 'year') {
                    $penalty = $charge->amount / 4;
                }
                if ($charge->penalty_period == 'month') {
                    $penalty = $charge->amount * 3;
                }
                if ($charge->penalty_period == 'week') {
                    $penalty = $charge->amount * 13;
                }
                if ($charge->penalty_period == 'day') {
                    $penalty = $charge->amount * 91.25;
                }
            }
            if ($loan->repayment_cycle == 'bi_monthly') {
                //return the penalty per bi-monthly
                if ($charge->penalty_period == 'year') {
                    $penalty = $charge->amount / 6;
                }
                if ($charge->penalty_period == 'month') {
                ...
                if ($charge->penalty_period == 'week') {
                    $penalty = $charge->amount * 8.67;
                }
                if ($charge->penalty_period == 'day') {
                    $penalty = $charge->amount * 58.67;
                }

            }

            if ($loan->repayment_cycle == 'monthly') {
                //return the penalty per monthly

                if ($charge->penalty_period == 'year') {
                    $penalty = $charge->amount / 12;
                }
                if ($charge->penalty_period == 'month') {
                    $penalty = $charge->amount * 1;
                }
                if ($charge->penalty_period == 'week') {
                    $penalty = $charge->amount * 4.33;
                }
                if ($charge->penalty_period == 'day') {
                    $penalty = $charge->amount * 30.4;
                }
            }
            if ($loan->repayment_cycle == 'weekly') {
                //return the penalty per weekly
                ...
                if ($charge->penalty_period == 'day') {
                    $penalty = $charge->amount * 7;
                }
            }
            if ($loan->repayment_cycle == 'daily') {
                //return the penalty per day
    ...
    //determine monthly payment using amortization
    // public static function amortized_monthly_penalty($id, $balance)
    // {
    //     $period = GeneralHelper::loan_period($id);
    //     $penalty_rate = GeneralHelper::determine_penalty_rate($id);
    //     //calculate here
    //     $amount = ($penalty_rate * $balance *$period);
    //     return $amount;
    // }
